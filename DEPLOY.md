...
## Черновой контент на проверке (таджвид)

Религиозный контент, ещё не проверенный специалистом (сейчас — черновой модуль
таджвида), спрятан за компайл-тайм флагом `MUSLINGO_DRAFT_CONTENT` (по умолчанию
выключен). В обычной прод-сборке его **нет**: пункта в профиле не видно, в курсы
он не добавляется (см. `lib/utils/flags.dart`, `lib/services/tajwid_data.dart`).

Чтобы специалист посмотрел черновик в приложении — собери с флагом:

```bash
# локально
/Users/alanbaimukhan/dev/flutter/bin/flutter run -d chrome --dart-define=MUSLINGO_DRAFT_CONTENT=true
# затем: Профиль → «Таджвид — на проверке (черновик)»
```

Экран ревью (`lib/screens/tajwid_review_screen.dart`) — read-only: показывает все
правила и вопросы с отмеченным верным ответом, ничего не засчитывает и не пишет на
сервер. Правила даны по стандартным учебникам; арабские примеры и тонкости
произношения помечены «(на проверке специалистом)» — их заполняет/правит
специалист. **В прод этот раздел не выкатывается, пока специалист не подтвердит.**
