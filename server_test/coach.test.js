...
test('parseCoachReply extracts reply, whitelisted action and sources', () => {
  const parsed = parseCoachReply(
    JSON.stringify({
      ...
      action: { type: 'startLesson', lessonId: 'lesson-7', label: 'Начать урок' },
      sources: ['Коран 1:1-7'],
    }),
  );
  assert.equal(parsed.text, 'Начни с суры Аль-Фатиха.');
  assert.deepEqual(parsed.action, { type: 'startLesson', lessonId: 'lesson-7', label: 'Начать урок' });
  assert.deepEqual(parsed.sources, ['Коран 1:1-7']);
});
