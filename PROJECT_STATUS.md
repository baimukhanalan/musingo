...
- **Курс: 61 урок** (арабский алфавит 16, Коран 35, основы 10) — арабский сверен с каноном.
...
### Контент / курс (61 урок)
- Арабский: полный алфавит a1–a16 (28 букв + танвин/шадда/мадд + чтение).
- Коран: Фатиха + короткие суры Джуз Амма (35 уроков), арабский **сверен построчно с каноном** `assets/data/quran-uthmani-tanzil.txt`.
...
Каждый шаг проверялся: `flutter analyze` (чисто), **flutter-тесты** (~86), **node-тесты** (138), `flutter build web`. Vercel гоняет те же тесты в `buildCommand` — «красная» сборка = упавший тест.
...
2. **Запушить код в GitHub:** вся работа сессии закоммичена локально (`git log` → верхний коммит), но **не запушена** — в окружении ассистента нет GitHub-креды (keychain пуст, токена нет). Нужен `git push origin main` из твоего терминала (или `gh auth login`, тогда ассистент сможет пушить сам).
...
- **Новые типы упражнений** Duolingo: перетаскивание слов в правильный порядок, аудирование-выбор (нужен новый LessonStepType + доработка `lesson_screen`).
- **Больше контента:** остальные суры Джуз Амма (78–84, 90, 93–100, 98), расширение основ/таджвида (после эксперта).
