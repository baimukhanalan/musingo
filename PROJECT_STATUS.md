...
**Текущий production deployment:** `dpl_DiDT7254ioytJEvRhVF91d8zS93q` (Ready)
...
Каждый шаг проверялся: `flutter analyze` (чисто), **Flutter-тесты** (102), **Node-тесты** (138), `flutter build web`. Vercel гоняет те же тесты в `buildCommand` — «красная» сборка = упавший тест.
