...
export default withApi(async (request, response) => {
  ...
  await consumeLoginAttempt(emailActionRateKey('verify-ip', ip), MAX_ATTEMPTS);
  await consumeLoginAttempt(emailActionRateKey('verify-address', `${email}|${ip}`), MAX_ATTEMPTS);
