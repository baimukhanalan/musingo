...
export async function callGroqTranscription({ audio, mimeType, prompt = '' }) {
  ...
  let httpResponse;
  try {
    httpResponse = await fetch(GROQ_TRANSCRIPTION_URL, {
      method: 'POST',
    ...
    throw new ApiError(503, 'speech_transcription_unavailable', 'Speech transcription failed.');
  } finally {
    clearTimeout(timer);
  }
  if (!httpResponse.ok) {
  ...
  try {
    ...
    throw new ApiError(503, 'speech_transcription_unavailable', 'Speech transcription returned invalid data.');
  }
